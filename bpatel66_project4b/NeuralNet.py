...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        for i in range(len(inActs)):
            newInActs.append(inActs[i])
        weightedSum = self.getWeightedSum(newInActs)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            ...
            newWeight = weight + (alpha * newInActs[i] * delta)
    ...
    def feedForward(self, inActs):
        ...
        for i in range(self.numLayers):
            ...
            for perceptron in layer:
                list.append(perceptron.sigmoidActivation(outputs[i]))
            outputs.append(list)
        return outputs
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for i in range(len(nextLayer)):
                        delta += nextLayer[i].weights[neuronNum+1] * deltas[layerNum][i]
                    hiddenDelta.append(delta * gPrime)
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        NNOutputs = nnet.feedForward(example[0])
        outputLayer = NNOutputs[-1]
        rounded = []
        for outputNode in outputLayer:
            rounded.append(round(outputNode))
        if rounded == example[1]:
            testCorrect = testCorrect + 1.0
        else:
