...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for transitions in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            nextStateValue = self.discount*probability*self.getValue(nextState)
            qVal += nextStateValue + nextStateReward
        return qVal
    ...
    def computeActionFromValues(self, state):
        ...
        if not self.mdp.isTerminal(state):
            ...
            bestQ = None
            for action in self.mdp.getPossibleActions(state):
                if bestQ < self.computeQValueFromValues(state, action) or bestQ == None:
                    bestAction = action
